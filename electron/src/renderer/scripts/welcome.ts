...
let currentPage = 1;
const totalPages = 4;
let selectedAccessMode: 'localhost' | 'network' | 'ngrok' = 'localhost';
...
function showPlatformInstructions(platform: NodeJS.Platform): void {
...
function setupEventListeners(): void {
  // Navigation buttons
  const navigationButtons: Array<{ id: string; action: () => void }> = [
  ...
  const accessModes = document.querySelectorAll<HTMLInputElement>('input[name="accessMode"]');
  accessModes.forEach(radio => {
    radio.addEventListener('change', (e) => {
      const target = e.target as HTMLInputElement;
      selectedAccessMode = target.value as typeof selectedAccessMode;
    });
  });
  
  // Launch at login checkbox
  const launchAtLoginCheckbox = document.getElementById('launchAtLogin') as HTMLInputElement;
...
// Export for testing
export { initialize, showPlatformInstructions, navigateToPage, finishSetup };