...
describe('SessionCreateForm', () => {
  ...
  describe('quick start buttons', () => {
    ...
    it('should update command when quick start is clicked', async () => {
      const pythonButton = Array.from(element.querySelectorAll('.grid button'))
        .find(btn => btn.textContent?.includes('python3'));
    ...
    it('should highlight selected quick start', async () => {
      ...
      const nodeButton = Array.from(element.querySelectorAll('.grid button'))
        .find(btn => btn.textContent?.includes('node'));
  ...
  describe('session creation', () => {
    ...
    it('should validate required fields', async () => {
      ...
      const createButton = Array.from(element.querySelectorAll('button'))
        .find(btn => btn.textContent?.trim() === 'Create') as HTMLButtonElement;
  ...
  describe('file browser integration', () => {
    it('should show file browser when browse button is clicked', async () => {
      const browseButton = element.querySelector('button[title*="📁"]') || 
                          element.querySelector('button:has-text("📁")') ||
                          Array.from(element.querySelectorAll('button')).find(btn => btn.textContent?.includes('📁'));
    ...
    it('should update working directory when directory is selected', async () => {
      ...
      const fileBrowser = element.querySelector('file-browser');
      if (fileBrowser) {
        fileBrowser.dispatchEvent(new CustomEvent('directory-selected', {
